...
for i in "${images[@]}"; do
    #echo "$i"
    #old_tag="registry.mayadata.io/"${i}":1.10.0-ee"
    #new_tag="registry.mayadata.io/"${i}":1.12.0-RC1.ee"
    #echo "$old_tag"
    # echo "$new_tag"
    #docker login https://registry.mayadata.io -u admin.mod@mayadata.io -p ~Maya_MOD@1658~
    #docker pull "${old_tag}"
    #docker tag "${old_tag}"  "${new_tag}"
    #docker push "${new_tag}"
    #awk -F':' '{ for(x=1;x<=NF;x++) print $x }' <<< $i
    #res="${var//[^/]}"
    #echo "${#res}"
    ...
    #echo "$tmp"
    #echo "$i"
    #docker login -u mayadata -p MayaDocker@123
    #docker login https://registry.mayadata.io -u admin.mod@mayadata.io -p ~Maya_MOD@1658~
    var2=$(echo $STR | cut -f2 -d-)
    docker pull "${i}"
    docker save -o ./${tmp}.tar ${i}
    echo "$tmp"
    else
    #echo "value is zero"
    #tmp=$(echo $i | cut -f"$(($count+1))" -d/)
...
cd ..
sudo tar -cvpf release_2.1.0.tar 2.1.0
sudo gzip --best release_2.1.0.tar
